...
export default function PrivacySettings({ userType, userData, steamUserData }: Props) {
  ...
  const handleSteamDelete = async () => {
    if (!steamUserData) return;

    setLoadingDelete(true);
    const results = await DeleteSteamAccountFrontend(steamUserData.id);
    setIsModalOpen(false);
    alert(results.message);
    setLoadingDelete(false);
    router.push('/');
